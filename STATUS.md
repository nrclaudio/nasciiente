...
*Last updated: 2026-07-14. Written so the project can be picked up cold, by
future-you or anyone else, after any amount of time away.*
...
implemented, tested (128 passing), and flag-gated; see TODO for what has
landed vs what awaits a GPU run. New decode flags on existing checkpoints:
`--order halton`, `--remask-eta`, `--critic-conf` (needs retrain), plus
training-side critic head, rare-caption upsampling, 30-epoch shading,
engine quality tags with sparse dialect routing, and
...
converted deterministically to glyphs, three style dialects per image).
Decoding is MaskGIT-style iterative unmasking. Full first-principles
documentation lives in [`docs/study-guide.md`](docs/study-guide.md):
architecture, training recipe, data engine, and all nine documented
failures with their fixes. Read that before touching anything; this file is
only the bookmark.
...
The hybrid decode (`bf91eb5`) runs the head uncapped, so layout forms under
gumbel exploration and rising guidance, and caps only the tail below mask
...
maximum-precision geometry). Then probe `final_model.pt` (add `--ema`) once
the human stage finishes, and ship.

Secondary open item: "a lighthouse" was mediocre even uncapped on the
latest shading checkpoint, and the older run drew it better. Could be seed
luck, where best-of-k at serve time is the intended mitigation, or a mild
...
- **Dropout 0.1 silently blocked prompt following** (`9fba787`), proven by
  a 64-grid memorization ablation (CE 1.0 vs 0.034).
...
- **Edge echo turned out to be parallel-commit multimodality in the
  schedule tail.** Fixed by the commit cap (`a83fc47`), then scoped to the
  tail only after full capping blank-collapsed sparse tonal prompts
  (`bf91eb5`). The rise-guidance schedule is keyed to step progress, not
  mask ratio (`faf57ef`).
...
4. Probes: `probe.py` (free generation), `probe_inpaint.py` (structure
   ...
   since the tail-capped decode beat it). All accept
   `--max-commit/--cap-below`.
...
Backups above are complete as of 2026-07-13. Destroy the GPU instance (it
bills hourly), revoke the instance's GitHub fine-grained token and any
write-scoped HF token. Nothing else leaks money or secrets; the project
resumes from step 1 above whenever.
